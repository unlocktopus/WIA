...
typedef struct {
	unsigned char value; //valor del byte
	int position;	//ordinal del byte en el payload
}BubbleUnit;
...
BubbleUnit encodedText[350];


void initBubbleUnit(unsigned char* plaintext, BubbleUnit* unit, unsigned int length) { //inicializa el arreglo de BubbleUnits con el payload 
	for (int i = 0; i < length; i++) {
		unit[i].value = plaintext[i];
		unit[i].position = i;
	}
}

void bubbleEncode(BubbleUnit* unit, unsigned int length) {
	bool finish = false;
	while (!finish) {
		finish = true;
		for (int i = 0; i < length - 2; i++) {
			if (unit[i].value > unit[i + 1].value) {
				finish = false;
				unit[i].value = unit[i].value ^ unit[i + 1].value; //se intercambian los valores con XOR swap
...
int main()
{
	unsigned int length = sizeof(plaintext);
	initBubbleUnit(plaintext, encodedText, length);
	bubbleEncode(encodedText, length); //codificacion del payload
	printf("%s", "BubbleUnit encodedPayload[] ={\n");
	for (int i = 0; i < sizeof(plaintext); i++) {
		printf("{0x%x,0x%x}", encodedText[i].value, encodedText[i].position);
